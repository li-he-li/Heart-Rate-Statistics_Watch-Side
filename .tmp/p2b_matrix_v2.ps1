...
$out='D:\MyApplication\.tmp\p2b_matrix_report_v2.md'
...
# B1.3 pause/resume
$bP=Alive $phone 'com.heartrate.phone' 'PhoneRelayForegroundService'; $bW=Alive $wear 'com.heartrate.wear' 'WearMonitoringForegroundService'
& $adb -s $phone shell input keyevent 3 | Out-Null
& $adb -s $wear shell input keyevent 3 | Out-Null
Start-Sleep 10
$hP=Alive $phone 'com.heartrate.phone' 'PhoneRelayForegroundService'; $hW=Alive $wear 'com.heartrate.wear' 'WearMonitoringForegroundService'
& $adb -s $phone shell monkey -p com.heartrate.phone -c android.intent.category.LAUNCHER 1 | Out-Null
& $adb -s $wear shell monkey -p com.heartrate.wear -c android.intent.category.LAUNCHER 1 | Out-Null
Start-Sleep 10
$aP=Alive $phone 'com.heartrate.phone' 'PhoneRelayForegroundService'; $aW=Alive $wear 'com.heartrate.wear' 'WearMonitoringForegroundService'
$cP=CrashHits $phone 'com.heartrate.phone'; $cW=CrashHits $wear 'com.heartrate.wear'
if($bP -and $bW -and $hP -and $hW -and $aP -and $aW -and $cP -eq 0 -and $cW -eq 0){Add 'B1.3-A pause/resume' 'PASS' 'services stable through home/resume, no process crash'} else {Add 'B1.3-A pause/resume' 'FAIL' "bP=$bP bW=$bW hP=$hP hW=$hW aP=$aP aW=$aW cP=$cP cW=$cW"}
...
& $adb -s $wear shell monkey -p com.heartrate.wear -c android.intent.category.LAUNCHER 1 | Out-Null
Start-Sleep 10
$rP=Alive $phone 'com.heartrate.phone' 'PhoneRelayForegroundService'; $rW=Alive $wear 'com.heartrate.wear' 'WearMonitoringForegroundService'
$cP=CrashHits $phone 'com.heartrate.phone'; $cW=CrashHits $wear 'com.heartrate.wear'
...
& $adb -s $wear shell monkey -p com.heartrate.wear -c android.intent.category.LAUNCHER 1 | Out-Null
Start-Sleep 10
$grantAlive=Alive $wear 'com.heartrate.wear' 'WearMonitoringForegroundService'
$cW=CrashHits $wear 'com.heartrate.wear'
if(((-not $denyAlive)) -and $grantAlive -and $cW -eq 0){Add 'B4-2 permission deny/regrant' 'PASS' 'denied->service off, regrant->service recovered, no process crash'} else {Add 'B4-2 permission deny/regrant' 'FAIL' "denyAlive=$denyAlive grantAlive=$grantAlive cW=$cW"}
...
$lines=@('# P2-B Matrix Report v2','',"Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')",'', '| Case | Status | Details |','|---|---|---|')
